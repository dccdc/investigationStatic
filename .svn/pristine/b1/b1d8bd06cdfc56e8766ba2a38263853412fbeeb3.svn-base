<template>
  <v-menu id="menu" transition="scale-transition" offset-x max-width="150px">
    <template v-slot:activator="{ on }">
		<!-- <button   v-on="on" class="space" @click="getItems()"		 style="color: #d21f1d;background-color: white;height: 44px;border: 0.2px solid #d1cdcd;outline: none;">{{title}}</button> -->
      <v-btn   tile large v-on="on" class="space" @click="getItems()" style="color: #d21f1d;">{{title}}</v-btn>
    </template>
